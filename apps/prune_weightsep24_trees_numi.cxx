    ...
    if (mcweight_filled>0) {
      ...
      if (SaveGenieFluxKnob=="UBGenieFluxSmallUni") {
        for(auto const& knob: UBGenieFluxSmallUni) {
         ...
         if ((weight_f2.size() > 0) && (weight_f2.size() < 1000)) { // extend to 1000 by inserting copies until it hits 1000 universes
           int nsize = weight_f2.size();
           int njump = (1000/nsize) - 1;
           int nremain = 1000 % (nsize);
           while (njump > 0) {
             weight_f2.insert(weight_f2.end(), weight_f2.begin(), weight_f2.begin()+nsize);
             njump--;
           }
           if (nremain > 0) {
             weight_f2.insert(weight_f2.end(), weight_f2.begin(), weight_f2.begin()+nremain);
           }
         }
         if ((weight_f2.size() > 0) && (weight_f2.size() != 1000)) { // extend to 1000 by inserting copies until it hits 1000 universes
           std::cerr << "Knob : " << SaveGenieFluxKnob << " has " << weight_f2.size() << " universes, not 1000 as expected!!" << std::endl;
         }
