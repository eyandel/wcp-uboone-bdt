...
int main( int argc, char** argv )
{
  ...
  else // flag_breakdown == true
  {
    ...
    for(auto it = map_obsch_subhistos.begin(); it!= map_obsch_subhistos.end(); it++){
        ...
        legend[obschannel-1] = new TLegend(0.15, 0.65, 0.7, 0.92);
        ...
        TH1F* hSPNp    = (TH1F*)hdata->Clone("hSPNp");
        TH1F* hSP0p    = (TH1F*)hdata->Clone("hSP0p");
        ...
        for(size_t i=0; i<it->second.size(); i++){
            ...
            for(std::string line; std::getline(sss, line, '_');){
                ...
                if(line == "SP0p") {
                    std::cout<<"SP0p"<<" "<<histname<<std::endl;
                    hSP0p->Add(htemp);
                    break;
                }
                if(line == "SPNp") {
                    std::cout<<"SPNp"<<" "<<histname<<std::endl;
                    hSPNp->Add(htemp);
                    break;
                }
                if(line == "LEE") {
      ...
      /*  hstack[obschannel-1]->Add(hRnumuCCinFV);
        ...
        hstack[obschannel-1]->Add(hSPNp);
        legend[obschannel-1]->AddEntry(hSPNp, Form("1#gamma Np, %.1f", hSPNp->Integral()), "F");
        hSPNp->SetFillStyle(1001);
        hSPNp->SetFillColorAlpha(kPink-1, 0.5);
        hSPNp->SetLineColor(kPink-1);
        hSPNp->SetLineWidth(1);

        hstack[obschannel-1]->Add(hSP0p);
        legend[obschannel-1]->AddEntry(hSP0p, Form("1#gamma 0p, %.1f", hSP0p->Integral()), "F");
        hSP0p->SetFillStyle(1001);
        hSP0p->SetFillColorAlpha(kPink+1, 0.5);
        hSP0p->SetLineColor(kPink+1);
        hSP0p->SetLineWidth(1);
